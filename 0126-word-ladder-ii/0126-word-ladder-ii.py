...
class Solution:
    def findLadders(self, beginWord: str, endWord: str, wordList: List[str]) -> List[List[str]]:
        ...
        # 如果 beginWord 和 endWord 只差一个字母，直接返回
        ...
        for depth in range(1, len_wordlist + 2):
            ...
            if intersection := begin.searching & end.searching:
                ...
                # 回溯构建路径
                res = [[i] for i in intersection]
                while res[0][0] != -2:
                    res = [[i] + row for row in res for i in begin.searched[row[0]]]
                while res[0][-1] != -1:
                    res = [row + [i] for row in res for i in end.searched[row[-1]]]
                
                return [[wordList[i] for i in row] for row in res]
